...
The Ensemble-Kalman-Inversion method addresses inverse problems, where the goal is to estimate model parameters $\mathbf{m}  \in \mathbb R^M$ that best explain observed data $\mathbf{d} \in \mathbb C^N$.
The forward problem is formulated as
...
**Initialization**
Draw $\{ m_{n}^{(j)}\}_{j=1}^J$ ensemble members from a prior distribution.
Set $\rho \in (0,1)$ and $\tau > 1/\rho$.
Then for $n=0,1 \dots$

**Prediction**
Evaluate $w_n^{(j)} = G(m_n^{(j)})$ and calculate the mean value $\overline{w}_n$.

**Termination**
If $||\Gamma^{-1/2}(\mathbf d - \overline w_n) || \le \tau \eta$, terminate and output the ensemble mean $\overline m_n$ as solution.

**Analysis**
At each iteration, an ensemble of perturbed data $\{ y_{n}^{(j)}\}_{j=1}^J$ is generated with additional noise $y_n^{(j)} = d + \eta$.
